// THIS PLAYER IS WORKING AS INTENDED AXCEPT FOR MOVEMENT DASHING
// INPUT SCRIPT ----------
sc_input();
// END INPUT SCRIPT ----------
...
if (dash == 1) {state = "dash"; image_index = 0; noinput = true; dash = 0;
...
/*
if dash {
	if left {x += sin(4*4) * 20};
	if right {x -= sin(4*4) * 20};
	if up {y += sin(4*4) * 20};
	if down {y -= sin(4*4) * 20};
